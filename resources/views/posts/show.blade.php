...
<span class="tag label label-primary" v-for="tag in tags"> @{{ tag.name }} </span>
<article>
    <p v-for="p in post.body.split('\n')"> @{{ p }} </p>
</article>

@if($post->isOwner())
<a href="{{ route('posts_edit', ['post' => $post->id]) }}" class="btn btn-primary">
    Edit
</a>
@endif
<hr>
<div>
        ...
        <section>
            ...
            <div v-if="isCommentOwner(comment.user)" class="hidden"
                ...
                <div class="form-group">
                    <div class="col-md-6">
                        <textarea v-model="comment.body" class="form-control" name="body" rows="3"></textarea>
                        <button @click="saveComment(comment)" class="btn btn-sm btn-primary">
                            Save
                        </button>
                    </div>
                </div>
            ...
            <div v-if="isCommentOwner(comment.user)" class="hidden"
                ...
                <div class="form-group">
                    <div class="col-md-6">
                        <button @click="confirmDeleteComment(comment)" class="btn btn-sm btn-primary">
                        Confirm
                        </button>
                    </div>
                </div>
    ...
    Add your comment:
    <form method="POST" action="{{ route('comments_add', ['post' => $post->id]) }}">
        {{ csrf_field() }}
